...
function Builder(dir, parent) {
  ...
  // add a configurable base path for assets (largely for locals)
  // @dominicbarnes -- 2/14/2014
  if (this.config.assetsBase) {
    this.assetsBase = this.config.assetsBase;
  }
...
Builder.prototype.copyAssetsTo = function(dir){
  ...
  this.assetsDest = conf.assetsBase ? join(dir, conf.assetsBase) : dir;
};
...
Builder.prototype.inherit = function(dep){
  ...
  if (this.assetsBase) {
    dep.assetsBase = this.assetsBase;
  }
...
function rewriteUrls(builder, file, css) {
  ...
  function rewrite(_, url) {
    ...
    // for `url.resolve()`
    var folder = [builder.urlPrefix, builder.assetsBase || '', name, dirname(file)].filter(empty).join('/') + '/';
